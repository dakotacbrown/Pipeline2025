...
def step_run_job(context):
    ...
    with patch.object(sys, "argv", context.argv), \
         patch("src.run_step.GithubConnection", return_value=fake_github), \
         patch("src.api_wrapper.run_ingester", return_value={"rows": 9}) as run_ingester, \
